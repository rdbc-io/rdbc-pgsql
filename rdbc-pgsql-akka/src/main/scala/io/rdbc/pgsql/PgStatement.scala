...
import io.rdbc.core.ImmutIndexedSeq
import io.rdbc.core.api.Statement
import io.rdbc.core.api.exceptions.BindException.{MissingParamValException, NoSuitableConverterFoundException}
...
trait PgStatement extends Statement {
  ...
  def convertParams(params: Map[String, Any]): Map[String, DbValue] = params.map { anyParamEntry =>
    ...
    typeConvRegistry.byClass(anyParamValue.getClass)
      .map(converter => {
...
class CachedPgStatement(val stmtName: String, val nativeStmt: PgNativeStatement, val typeConvRegistry: PgTypeConvRegistry)(implicit val sessionRef: SessionRef, system: ActorSystem, materializer: ActorMaterializer) extends PgStatement {

  override protected def parametrizedStmt(dbValues: ImmutIndexedSeq[DbValue]): PgParametrizedStatement = {
    new CachedPgParametrizedStatement(stmtName, dbValues, typeConvRegistry)
  }
}

class NotCachedPgStatement(val nativeStmt: PgNativeStatement, val stmtName: Option[String], val typeConvRegistry: PgTypeConvRegistry)(implicit val sessionRef: SessionRef, system: ActorSystem, materializer: ActorMaterializer) extends PgStatement {

  override protected def parametrizedStmt(dbValues: ImmutIndexedSeq[DbValue]): PgParametrizedStatement = {
    new NotCachedPgParametrizedStatement(nativeSql, stmtName, dbValues, typeConvRegistry)
  }
}

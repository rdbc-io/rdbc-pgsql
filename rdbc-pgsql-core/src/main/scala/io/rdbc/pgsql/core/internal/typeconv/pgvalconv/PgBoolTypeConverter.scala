...
private[typeconv] object PgBoolTypeConverter
  extends PartialTypeConverter[PgBool] {
  val cls = classOf[PgBool]

  def convert(any: Any): Option[PgBool] = {
    BoolTypeConverter.convert(any).map(PgBool)
  }
}

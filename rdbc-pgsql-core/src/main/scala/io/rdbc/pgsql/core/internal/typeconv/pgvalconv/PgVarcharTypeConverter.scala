...
private[typeconv] object PgVarcharTypeConverter
  extends PartialTypeConverter[PgVarchar] {

  val cls = classOf[PgVarchar]

  def convert(any: Any): Option[PgVarchar] = {
    StringTypeConverter.convert(any).map(PgVarchar)
  }
}

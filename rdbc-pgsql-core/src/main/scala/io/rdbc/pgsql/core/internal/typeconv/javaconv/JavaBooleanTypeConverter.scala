...
private[typeconv] object JavaBooleanTypeConverter
  extends PartialTypeConverter[java.lang.Boolean] {
  val cls = classOf[java.lang.Boolean]

  def convert(any: Any): Option[lang.Boolean] = {
    BoolTypeConverter.convert(any).map(boolean2Boolean)
  }
}

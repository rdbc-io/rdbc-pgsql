...
private[typeconv] object LocalDateTimeTypeConverter
  extends PartialTypeConverter[LocalDateTime] {

  val cls = classOf[LocalDateTime]

  def convert(any: Any): Option[LocalDateTime] = {
    any match {
      case LocalDateTimeVal(ldt) => Some(ldt)
      case _ => None
    }
  }
}

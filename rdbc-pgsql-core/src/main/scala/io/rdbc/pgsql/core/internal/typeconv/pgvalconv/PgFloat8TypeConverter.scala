...
private[typeconv] object PgFloat8TypeConverter
  extends PartialTypeConverter[PgFloat8] {

  val cls = classOf[PgFloat8]

  def convert(any: Any): Option[PgFloat8] = {
    DoubleTypeConverter.convert(any).map(PgFloat8)
  }
}

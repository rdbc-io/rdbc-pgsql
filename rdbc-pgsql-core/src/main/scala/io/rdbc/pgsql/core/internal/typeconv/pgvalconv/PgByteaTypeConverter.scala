...
private[typeconv] object PgByteaTypeConverter
  extends PartialTypeConverter[PgBytea] {
  val cls = classOf[PgBytea]

  def convert(any: Any): Option[PgBytea] = {
    ByteVectorTypeConverter.convert(any).map(PgBytea)
  }
}

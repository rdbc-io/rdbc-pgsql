...
private[typeconv] object PgDateTypeConverter
  extends PartialTypeConverter[PgDate] {

  val cls = classOf[PgDate]

  def convert(any: Any): Option[PgDate] = {
    LocalDateTypeConverter.convert(any).map(PgDate)
  }
}

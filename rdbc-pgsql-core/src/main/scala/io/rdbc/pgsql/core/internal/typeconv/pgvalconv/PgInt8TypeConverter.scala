...
private[typeconv] object PgInt8TypeConverter
  extends PartialTypeConverter[PgInt8] {

  val cls = classOf[PgInt8]

  def convert(any: Any): Option[PgInt8] = {
    LongTypeConverter.convert(any).map(PgInt8)
  }
}

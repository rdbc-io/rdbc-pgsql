...
private[typeconv] object PgTimeTypeConverter
  extends PartialTypeConverter[PgTime] {

  val cls = classOf[PgTime]

  def convert(any: Any): Option[PgTime] = {
    LocalTimeTypeConverter.convert(any).map(PgTime)
  }
}

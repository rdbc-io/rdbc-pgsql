...
private[typeconv] object PgFloat4TypeConverter
  extends PartialTypeConverter[PgFloat4] {

  val cls = classOf[PgFloat4]

  def convert(any: Any): Option[PgFloat4] = {
    FloatTypeConverter.convert(any).map(PgFloat4)
  }
}

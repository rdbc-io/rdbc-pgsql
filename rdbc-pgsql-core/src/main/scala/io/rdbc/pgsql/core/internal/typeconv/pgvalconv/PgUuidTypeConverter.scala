...
private[typeconv] object PgUuidTypeConverter
  extends PartialTypeConverter[PgUuid] {

  val cls = classOf[PgUuid]

  def convert(any: Any): Option[PgUuid] = {
    UuidTypeConverter.convert(any).map(PgUuid)
  }
}
